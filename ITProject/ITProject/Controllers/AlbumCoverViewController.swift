...
class AlbumCoverViewController: UIViewController
{
	...
	private func loadAlbumCollectionView()
	{
		...
		self.albumCollectionView.register(UINib(nibName: AlbumCoverViewController.NIB_NAME, bundle: nil), forCellWithReuseIdentifier: AlbumCoverViewController.CELL_IDENTIFIER)
		let layout = self.albumCollectionView!.collectionViewLayout as! UICollectionViewFlowLayout
		layout.itemSize = CGSize(width: self.albumCollectionView.frame.size.width, height: self.albumCollectionView.bounds.size.height / 2.2)
		self.albumCollectionView?.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 9, right: 0)
	}
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?)
	{
		if segue.identifier == Storyboard.showAlbumDetail
		{
			...
			{
				let selectedAlbum = self.albumsList.getAlbum(index: (sender as! IndexPath).row)
				...
				CacheHandler.getInstance().getAllPhotosInfo(currAlbum: albumUID)
				{ detail, error in
					if let error = error
					{
...
extension AlbumCoverViewController: UICollectionViewDelegate, UICollectionViewDataSource, SwipeCollectionViewCellDelegate
{
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
	{
		...
		cell.album = self.albumsList.getAlbum(index: indexPath.item)
		cell.delegate = self
		return cell
	}
