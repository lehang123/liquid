...
class Util
{
	                                      ...
	                                      errorHandler: @escaping (Error?) -> Void = { _ in })
	{
		...
		let fileFullPath = self.GetFullFilePath(fileName: fileFullName)
		self.WriteFileToDocumentDirectory(data: data, fileFullName: fileFullPath)
		{
			fileUrl in
			...
			ZipFile(from: folderPath.absoluteString as NSString,
			        ...
			        fileName: fileName, fextension: fExtension, deleteAfterFinish: true)
			{ zipFileUrl in
				...
				ReadFileFromDocumentDirectory(fileName: filePath)
				{ fileData in
					let sRef = storageRef.child(filePath)

					// Upload the file to the path "images/rivers.jpg"
					_ = sRef.putData(fileData, metadata: metadata)
					{ metadata, error in
	                                ...
	                                errorHandler: @escaping (Error?) -> Void = { _ in })
	{
		if let imageid = imageUID, let imageExt = UIDExtension
		{
			...
			else
			{
				...
				else if self.GetDataFromLocalFile(filename: imageid, fextension: imageExt, completion: completion)
				{
				...
				else
				{
					...
					self.GetImageFromServer(imageUID: imageid, completion: completion, errorHandler: errorHandler)
				}
			}
		}
		else
	                                      ...
	                                      errorHandler: @escaping (Error?) -> Void = { _ in })
	{
		...
		self.DownloadFileFromServer(fileFullPath: fullFliePath, completion: { url in
	                                          ...
	                                          errorHandler: @escaping (Error?) -> Void = { _ in })
	{
		...
		let gsReference = Storage.storage().reference(forURL: self.FIREBASE_STORAGE_URL + fileFullPath)
		...
		_ = gsReference.write(toFile: localURL)
		{ url, error in
			if let error = error
			{
				// Uh-oh, an error occurred!
	...
	{
		let fileFullPath = self.GetFolderByExtension(fextension: fextension, withPathSlash: true)! + fileName + "." + Util.EXTENSION_ZIP
		let desertRef = Storage.storage().reference(forURL: self.FIREBASE_STORAGE_URL + fileFullPath)
	...
	{
		let fileFullPath = self.GetFullFilePath(fileName: fileName)
		print("GetMetadataFromServer :file path " + fileFullPath)
		let gsReference = Storage.storage().reference(forURL: self.FIREBASE_STORAGE_URL + fileFullPath)

		// Get metadata properties
		gsReference.getMetadata
		{ metadata, error in
			if let error = error
			{
				// Uh-oh, an error occurred!
	                             ...
	                             completion: @escaping ((URL) -> Void) = { _ in })
	{
		...
		unzippedFile = (unzippedFile as NSString).appendingPathExtension(self.GetExtensionByFolderPath(fileFullPath: to as String)!)!
	...
	public static func GetDataFromLocalFile(filename: String, fextension: String, completion: @escaping (Data) -> Void = { _ in }) -> Bool
	{
		...
		let folderPath = self.GetFolderByExtension(fextension: fextension, withPathSlash: true)!
		let fileDocumentFullPath = self.GetDocumentsDirectory().appendingPathComponent(folderPath, isDirectory: true).absoluteString as NSString
		...
		if self.DoesFileExist(fullPath: fileFullPath)
		{
			print("GetDataFromFile : oringinal file exist, no need for unzip :" +
				fileFullPath)
			self.ReadFileFromDocumentDirectory(fileName: self.GetFullFilePath(fileName: filenameWithExtension!))
			{
				data in
		...
		{ // if not, find a zip file, unzip and then get data
			let zipFilename = URL(string: filename)?.appendingPathExtension(self.EXTENSION_ZIP).absoluteString
			...
			if self.DoesFileExist(fullPath: zipFileFullPath)
			{
				self.UnzipFile(from: fileDocumentFullPath, to: fileDocumentFullPath, fileName: zipFilename!, deleteAfterFinish: false)
				{ url in
	...
	{
		return self.GetFolderForFile(fileName: fileName, withPathSlash: true)
	}

	public static func GetFolderForFile(fileName: NSString, withPathSlash: Bool) -> String?
	{
		return self.GetFolderByExtension(fextension: "." + fileName.pathExtension,
	...
	{
		if fextension == self.EXTENSION_JPEG ||
			fextension == ("." + self.EXTENSION_JPEG) ||
			fextension == self.EXTENSION_PNG ||
			fextension == ("." + self.EXTENSION_PNG)
		{
			return withPathSlash ? (self.IMAGE_FOLDER + "/") : self.IMAGE_FOLDER
		}
		return nil
	}
	...
	{
		return self.GetDocumentsDirectory().appendingPathComponent(self.GetFolderForFile(fileName: fileName as NSString)! + fileName).absoluteString
	}
	...
	{
		return self.GetFolderForFile(fileName: fileName as NSString)! + fileName
	}
	...
	{
		return self.GetDocumentsDirectory().appendingPathComponent(self.IMAGE_FOLDER, isDirectory: true)
	}

	public static func GetTmpDirectory() -> URL
	{
		return self.GetDocumentsDirectory().appendingPathComponent(self.TMP_FOLDER, isDirectory: true)
	}
		...
		if !FileManager.default.fileExists(atPath: folderURL.absoluteString, isDirectory: &isDir)
		{
			do
			{
				...
				print("CreateFolderInDocuments: Created " + folderURL.absoluteString + " success : "
					+ String(self.DoesDirectoryExist(fullPath: folderURL.absoluteString)))
			}
			catch
			{
	...
	{
		let dataPath = self.GetDocumentsDirectory().appendingPathComponent(self.IMAGE_FOLDER, isDirectory: true)
		self.CreateFolderInDocuments(folderURL: dataPath)
	}

	/// Create the tmp folder for later use
	public static func CreateTmpFolder()
	{
		let dataPath = self.GetDocumentsDirectory().appendingPathComponent(self.TMP_FOLDER, isDirectory: true)
		self.CreateFolderInDocuments(folderURL: dataPath)
	}
	...
	{
		return self.DoesFileExist(fullPath: fullPath, checkIsDirectory: false)
	}
	...
	{
		return self.DoesFileExist(fullPath: fullPath, checkIsDirectory: true)
	}
	                             ...
	                             aaction: @escaping (() -> Void) = {})
	{
		...
		DispatchQueue.main.async
		{
			...
			alertController.addAction(UIAlertAction(title: action_title, style: .default)
				{ _ in
					aaction()
			})
