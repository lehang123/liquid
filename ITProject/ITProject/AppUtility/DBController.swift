...
class DBController
{
	...
	{
		self.getDocumentReference(collectionName: collectionName, documentUID: documentUID).setData(inputData)
		{ err in
			if let err = err
			{
	...
	public func addDocumentToCollection(inputData: [String: Any], collectionName: String) -> DocumentReference
	{
		...
		ref = self.db.collection(collectionName).addDocument(data: inputData)
		{ err in
			if let err = err
			{
	...
	{
		self.getDocumentReference(collectionName: collectionName, documentUID: documentUID).delete
		{ err in
			if let err = err
			{
	...
	public func incrementValue(increment: Int64, fieldName: String, documentUID: String, collectionName: String)
	{
		self.getDocumentReference(collectionName: collectionName, documentUID: documentUID).updateData([
