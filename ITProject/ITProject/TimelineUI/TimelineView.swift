...
class TimelineView: UIView
{
	...
	private func setupContent()
	{
		...
		let viewFromAbove = self.setupGuideView()

		self.setupTimeline(viewFromAbove: viewFromAbove)
	}
	...
	private func setupTimelinePoint(_ width: CGFloat) -> UIView
	{
		...
		let shapeLayer = self.drawCirclePoint(pointWidth: width)
	...
	fileprivate func blockForTimeFrame(_ element: TimelineField, isFirst: Bool = false, isLast: Bool = false) -> UIView
	{
		let v = UIView()
		v.translatesAutoresizingMaskIntoConstraints = false
		v.addConstraint(NSLayoutConstraint(item: v, attribute: .height, relatedBy: .greaterThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: self.bulletSize))

		// bullet
		let bullet: UIView = self.setupTimelinePoint(self.bulletSize)
		v.addSubview(bullet)
		v.addConstraints([
		...
		self.configureDateLabel(dateLabel)
		...
		{
			v.addConstraint(NSLayoutConstraint(item: line, attribute: .height, relatedBy: .equal, toItem: v, attribute: .height, multiplier: 1.0, constant: -self.bulletSize))
		}
	...
	private func setupTimeline(viewFromAbove: UIView)
	{
		...
		for (index, element) in self.timelineField.enumerated()
		{
			let v = self.blockForTimeFrame(element, isFirst: index == 0, isLast: index == self.timelineField.count - 1)
			addSubview(v)
			addConstraints([
